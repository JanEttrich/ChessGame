...
class MoveGenerationTest {
    ...
    void testStartPosition(int expectedPositionCount, int depth) {
        game = new GameInTerminal(null, true);
    ...
    void testPos2(int expectedPositionCount, int depth) {
        game = new GameInTerminal("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R", true);
        assertEquals(expectedPositionCount, positionCounter(depth, true));
    }
    ...
    void testPos3(int expectedPositionCount, int depth) {
        game = new GameInTerminal("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8", true);
        assertEquals(expectedPositionCount, positionCounter(depth, true));
    }
    ...
    void testPos4(int expectedPositionCount, int depth) {
        game = new GameInTerminal("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1", true);
        assertEquals(expectedPositionCount, positionCounter(depth, true));
    }
    ...
    void testPos5(int expectedPositionCount, int depth) {
        game = new GameInTerminal("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R", true);
        assertEquals(expectedPositionCount, positionCounter(depth, true));
    }
    ...
    void testPos6(int expectedPositionCount, int depth) {
        game = new GameInTerminal("r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1", true);
        assertEquals(expectedPositionCount, positionCounter(depth, true));
    }


    private int positionCounter(int depth, boolean white) {
        if (depth == 0) {
            return 1;
        }

        List<Move> moves = game.generate(white);
        int numPositions = 0;
        ...
        for (Move move : moves) {
            ...
            numPositions += positionCounter(depth - 1, game.getActivePlayer().isWhite());
